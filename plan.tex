...
% ============================================================
% IMPLEMENTATION SKELETON — Part I of the paper (the framework)
% Build order: skeleton first, then fill section by section.
% Each subsection carries a % FILL: note describing intended content.
% Now implemented in src.tex; kept here as the design blueprint.
% ============================================================
\clearpage
\begin{center}
{\color{accentdark}\rule{\textwidth}{1.2pt}}\\[6pt]
{\Huge\bfseries\color{accentdark} Implementation Skeleton}\\[8pt]
{\LARGE\bfseries\color{accentdark!85!black} Part~I --- A Governance Framework for Safe Adoption}\\[8pt]
{\color{accentdark}\rule{\textwidth}{1.2pt}}
\end{center}
\vspace{8pt}

\begin{quotebox}
\textbf{Guiding question (Part~I).} How can organizations adopt GenAI-assisted document redaction safely, through governance, human-in-the-loop controls, audit logging and risk management?
\end{quotebox}

% --- Orientation note for the author ---
\begin{tcolorbox}[
  enhanced, breakable, colback=gray!6, colframe=gray!40, boxrule=0.6pt, arc=2pt,
  left=8pt, right=8pt, top=5pt, bottom=5pt,
  title={\bfseries How Part~I is built},
  coltitle=gray!60!black,
  attach boxed title to top left={yshift=-2mm, xshift=6mm},
  boxed title style={colback=gray!15, arc=2pt, boxrule=0.6pt, colframe=gray!40},
]
The guiding question is a \emph{design} question, so it is answered by an artefact: a governance framework plus an adoption model. Part~I runs \textbf{Foundations} (define ``safe adoption'' and anchor it to standards) $\rightarrow$ the \textbf{four pillars} taken directly from the question (governance, human-in-the-loop, audit logging, risk management) $\rightarrow$ \textbf{adoption, measurement and business case} $\rightarrow$ a \textbf{synthesis} of limitations and assumptions.\\[4pt]
\textit{Status: this skeleton is implemented in \texttt{src.tex}; the earlier proposal sections (Proposed Solution $\ldots$ Business Case) have been deepened and merged into the matching subsections there. The Post Office case is the paper's Part~II.}
\end{tcolorbox}
...
% FILL: research logic — the guiding question is design-oriented; the deliverable that answers it is a framework + adoption model. State Part I structure and map each pillar to a clause of the question.
\textit{[To be developed: research logic and structure of Part I.]}
...
% FILL: phased rollout Assess -> Design -> Pilot -> Scale (PM lifecycle; note iterative/Agile pilot from the course). Milestones and gate criteria. Includes the WBS / PERT / critical-path scheduling of the Design phase. (In src.tex, change management and training fold in here as WP4 + the "lack of staff training" risk R7, not a standalone subsection.)
\textit{[To be developed.]}
...
% FILL: assumptions (competent tool, engaged reviewers, vendor honours agreement, governance resourced) and limits (not zero risk, designed not validated, bounded scope). In src.tex the framework is traced back to the five safe-adoption criteria across the pillars, not in a separate "answer" section; the bridge to the Post Office case is the paper's Part II.
\textit{[To be developed: assumptions and limits.]}
