...
\textbf{42001:2023} \cite{iso42001} (AI management): pre-deployment risk assessment (Cl.~6), documented human oversight (A.6), data-subject impact evaluation (A.5). \textbf{27001:2022} \cite{iso27001} (information security): information classification (A.5.12), data masking (A.8.11), logging (A.8.15). The pillar that meets each clause is in Table~\ref{tab:standards}.
...
Information classification & ISO/IEC 27001 A.5.12 & Pillar~II (document risk tiers) \\
...
\begin{itemize}
  ...
  \item \textbf{Exposure}: the number of data subjects affected: 1~$=$ a single subject; 2~$=$ a small group (2--99); 3~$=$ 100 or more.
  \item \textbf{Audience}: the reach on release: 1~$=$ internal; 2~$=$ a named external party; 3~$=$ the public web.
\end{itemize}
\end{definitionbox}

The score (1 to 27) maps to the three tiers of Section~\ref{sec:architecture}: \textbf{standard} ($\leq 4$), \textbf{high} ($6$ to $12$), \textbf{very high} ($\geq 18$) --- the score-and-threshold logic the course applies in its Probability and Impact Matrix \cite{falchi_slides}. Override: any \emph{public} document (Audience~$=3$) is floored at \textbf{high}, since public release is irreversible. The Post Office document scored the maximum ($3\times3\times3=27$); Part~II shows the framework would have flagged it very high.
...
In PM terms, classification is Probability-and-Impact-Matrix prioritization and escalation is structured decision-making \cite{falchi_slides}.
...
Pillar~IV manages the risk of the tool itself. Automation removes some risk (human inconsistency, volume misses) but adds new, \emph{secondary} risks \cite{falchi_slides}: automation bias, vendor exposure, false confidence. The PMI process identifies, scores, treats, and monitors them \cite{falchi_slides}.
...
\begin{itemize}
  ...
  \item \textbf{Qualitative analysis}: the Probability and Impact Matrix \cite{falchi_slides}.
  \item \textbf{Quantitative analysis}: EMV and a decision tree for the costliest risks \cite{falchi_slides}.
...
Each risk is written in the course's \emph{cause--risk--effect} formulation \cite{falchi_slides}; the category column is the project's Risk Breakdown Structure (RBS), so responses target causes. P and I are \emph{inherent} ratings (pre-control); residuals are in Table~\ref{tab:responses}.

\begin{table}[H]
\centering
\footnotesize
\rowcolors{2}{accentlight!30!white}{white}
\begin{tabular}{@{} c p{2.3cm} p{2.5cm} p{3.0cm} p{2.9cm} c c @{}}
\toprule
\textbf{\color{accentdark}ID} & \textbf{\color{accentdark}Category (RBS)} & \textbf{\color{accentdark}Cause} & \textbf{\color{accentdark}Risk (uncertain event)} & \textbf{\color{accentdark}Effect} & \textbf{\color{accentdark}P} & \textbf{\color{accentdark}I} \\
\midrule
R1 & Detection & Model limits; novel data formats & Sensitive data not detected (false negative) & Unredacted PII published & M & H \\
R2 & Process & No version control; manual handling & Wrong document version published & Outdated, uncontrolled disclosure & M & H \\
R3 & Human factor & Reviewer defers to the tool & Over-reliance on AI (automation bias) & Tool misses pass unchecked & M & H \\
R4 & Compliance & No persistent record of decisions & Cannot demonstrate compliance & Regulatory finding; no defence & M & H \\
R5 & Vendor / data & Text sent off-premise for inference & Document text exposed to external vendor & Secondary data breach & M & H \\
R6 & Quality & Conservative model tuning & Excess false positives & Overprocessing; eroded trust & H & M \\
R7 & Organizational & Rushed rollout & Staff untrained; process bypassed & Controls not applied & M & M \\
R8 & Governance & No defined decision rights & Unclear accountability for release & No owner, no gate & M & H \\
\bottomrule
\end{tabular}
\caption{Risk register in cause--risk--effect form (inherent probability P and impact I, before controls).}
\label{tab:riskregister}
\end{table}

\subsection{Qualitative and Quantitative Analysis}

The \textbf{Probability and Impact Matrix} \cite{falchi_slides} shows the profile (Table~\ref{tab:heatmap}). The cluster in the medium-probability / high-impact cell justifies a control-heavy framework: these risks are neither rare nor mild.
...
\bottomrule
\end{tabular}
\caption{Probability and Impact Matrix of the inherent risk register.}
\label{tab:heatmap}
\end{table}

The costliest risks are weighed by \textbf{expected monetary value} (EMV~$=$~probability~$\times$~impact, summed over the register) and a decision tree whose branches are compared on EMV \cite{falchi_slides} --- the very tools the course applies to regulatory, compliance and litigation risk, which is precisely this domain. A control pays off when the expected loss it removes exceeds its cost. Two caveats: EMV is an average, and an average hides the tail (the flaw of averages), so moving \emph{from average values to distributions} by Monte Carlo simulation is what would reveal it \cite{falchi_slides}; this is why the very-high tier uses dual control rather than a cost-benefit test. These figures are illustrative, the case-calibrated EMV being deferred to the Part~II counterfactual (Section~\ref{sec:counterfactual}).
...
All four strategies appear: most risks are \emph{mitigated}; vendor liability is partly \emph{transferred} by contract (R5); untrained-staff exposure is \emph{avoided} (R7); irreducible false positives are \emph{accepted} (R6) --- deliberately, to avoid \emph{overmitigation}, since over-tuning the detector only multiplies false positives and overprocessing \cite{falchi_slides}. Residual risk is monitored via the audit review and KPIs, with \textbf{triggers} (catch rate below target, or any released-without-approval event) escalating to the Board, backed by a \emph{contingency reserve} for the known risks in the register and a \emph{management reserve}, owned by senior management, for the unknown-unknowns \cite{falchi_slides}. The claim is not zero risk but controlled risk: every inherent risk is moved out of the red cell of the Probability and Impact Matrix (Table~\ref{tab:heatmap}) and kept there.
...
The Design phase decomposes, via the WBS, into four work packages \cite{falchi_slides}: (WP1) vendor contracting and DPA; (WP2) pipeline integration and log infrastructure; (WP3) policy, RACI and decision rights; (WP4) reviewer training and certification. WP1 is the longest-lead item and gates WP2, so the \textbf{critical path} runs WP1~$\to$~WP2~$\to$~Pilot. Durations on the uncertain packages (integration, training) use the course's three-point PERT estimate on a beta distribution, $t_e=(o+4m+p)/6$ \cite{falchi_slides}, to size a realistic schedule and a time buffer ahead of the pilot gate.

\subsection{KPIs and Control Effectiveness}

KPIs answer one question: do the controls actually operate? They split into \textbf{leading} indicators --- the course's \emph{key control indicators}, tracking whether the critical controls run \cite{falchi_slides} --- and \textbf{lagging} indicators, the \emph{key risk indicators} that track outcomes against the organization's risk appetite \cite{falchi_slides}. Each ties to a safe-adoption criterion and feeds the Pillar~IV triggers (Section~\ref{sec:risk}).
...
This is the project \textbf{business case} \cite{falchi_slides}, reframed: \textbf{risk-driven, not cost-saving driven}. Adopting the framework \emph{changes} the organization rather than running it \cite{falchi_slides}, so it is a project to be justified, not an operating cost to be minimized: direct annual cost may rise, while the value is the expected loss avoided, so the justification is a risk-adjusted return on investment rather than a cheaper-processing saving. Assume 200 externally published documents per year.
...
The publication step is a \textbf{decision tree} \cite{falchi_slides} (Figure~\ref{fig:tree}): a decision node with two branches, control on or control off, each leading to a chance node where a breach either occurs or does not, the branches compared on expected value. Folding the tree back needs two inputs: the annual probability of a publication-stage breach and its impact. Both are illustrative, chosen consistently with the Part~I figures; what matters is the structure and the order of magnitude, not precision (Section~\ref{sec:synthesis}).
...
\begin{tikzpicture}[
  ...
  dec/.style={draw=accentdark, fill=accentdark, minimum size=12pt, inner sep=0pt},
  chance/.style={draw=accentdark, fill=white, circle, minimum size=12pt, inner sep=0pt},
  leaf/.style={draw=accentdark!35, fill=accentlight!45, rounded corners=2pt, inner sep=3pt, font=\scriptsize},
  lbl/.style={fill=white, inner sep=1.5pt, font=\scriptsize}
]
\node[dec] (D) at (0,0) {};
\node[text=accentdark, font=\scriptsize\bfseries, below=2pt of D] {Publish};

\node[chance] (off) at (4.3,1.7) {};
\node[chance] (on)  at (4.3,-1.7) {};
\node[text=accentdark, above=1pt of off, font=\scriptsize] {Status quo\\EMV $=$ \textbf{EUR 100k/yr}};
\node[text=accentdark, below=1pt of on, font=\scriptsize] {With framework\\EMV $=$ \textbf{EUR 10k/yr}};

\draw[->, accentdark, thick] (D) -- node[lbl, sloped, above] {control OFF} (off);
\draw[->, accentdark, thick] (D) -- node[lbl, sloped, below] {control ON} (on);

\node[leaf] (offb) at (9.4,2.5) {breach 20\% $\rightarrow$ $-$EUR 500k};
\node[leaf] (offn) at (9.4,1.1) {no breach 80\% $\rightarrow$ EUR 0};
\node[leaf] (onb)  at (9.4,-1.1) {breach 2\% $\rightarrow$ $-$EUR 500k};
\node[leaf] (onn)  at (9.4,-2.5) {no breach 98\% $\rightarrow$ EUR 0};

\draw[->, accentdark!55] (off) -- (offb);
\draw[->, accentdark!55] (off) -- (offn);
\draw[->, accentdark!55] (on) -- (onb);
\draw[->, accentdark!55] (on) -- (onn);
\end{tikzpicture}
\caption{Decision tree for the publication step, branches compared on EMV. Expected loss avoided $=$ EUR 90k/yr; net of the EUR 7k incremental direct cost (Table~\ref{tab:businesscase}), net expected benefit $\approx$ EUR 83k/yr.}
\label{fig:tree}
\end{figure}

\noindent\textbf{Impact.}\par
A serious public disclosure of special-category data for hundreds of subjects carries regulatory exposure, remediation and notification cost, possible compensation, and reputational damage. In this case the ICO issued only a \emph{reprimand}: it weighed a fine of up to about GBP~1.1\,million but judged the breach not ``egregious'' under its public-sector approach \cite{ico2025}, while the Post Office still paid compensation and identity-protection costs to those exposed. A private controller would more plausibly face the fine, so, calibrated to a generic organization, an illustrative EUR 500,000 per event is conservative against the GDPR maximum.
...
EMV is a mean, and a mean hides the tail, the flaw of averages \cite{falchi_slides}: \emph{the devil is in the tail}. The danger is not a steady drip of EUR 100,000 a year; it is the single catastrophic disclosure, which is exactly the event that occurred. This sits in the tail the course warns about --- a Black-Swan-class event \cite{falchi_slides}: rare, severe, and obvious only in hindsight (the ICO called it ``entirely preventable'' \cite{ico2025}). The course's prescription for such events is to \emph{narrow the window} they can pass through \cite{falchi_slides}, and the mandatory gate is exactly that narrowing. A low-frequency but irreversible risk is not governed by a cost-benefit average, which is why the very-high tier uses dual control rather than an EMV test (Section~\ref{sec:risk}): the framework is justified not because expected value is positive, but because the tail outcome is one the organization cannot accept. The Post Office case is that tail made real.
...
\newpage
% ============================================================
\section{Proof of Concept: Validating the Controls}
\label{sec:prototype}

To show the framework is operational and not only conceptual, a small proof of concept exercises the Pillar~II and Pillar~III pipeline end to end on a seeded sample document --- the \emph{verification-and-validation} step that moves a predictive design toward a validated deliverable before the pilot of Section~\ref{sec:adoption} \cite{falchi_slides}.

Run against a draft memo flagged for public release that holds the bank and identity data of two named individuals, the pipeline behaves exactly as the framework prescribes. The document is classified automatically; sensitivity, exposure and the public-web audience score it into the \textbf{very-high} tier; and it is routed to that tier's control --- \textbf{independent review plus dual approval}, with no automatic release. The audit record is written and \textbf{publication stays blocked until a human clears the gate}: no record, no token, no release. Figure~\ref{fig:redacted} shows the redacted output, every detected item removed while the narrative is preserved, so the document stays useful.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=1.0\textwidth]{redaction-demo/output/redacted_document.pdf}}
\caption{Proof-of-concept output: each detected personal or financial item is removed and replaced by a labelled bar, while the narrative (here, the absence of a technical audit before recovery) is preserved.}
\label{fig:redacted}
\end{figure}

The proof of concept is deliberately partial in one telling way: an untitled name slips past automatic detection. That miss is the point --- it makes the automation-bias argument concrete. Detection is a first pass with partial recall, so the human reviewer must catch what the tool leaves, which is why no tier permits automatic release (Section~\ref{sec:hitl}). The negative result is as instructive as the positive: detection is an aid to a human gate, not a replacement for it.
